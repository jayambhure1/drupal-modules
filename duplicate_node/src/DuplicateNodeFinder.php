...
class DuplicateNodeFinder {
  ...
  public function findNodeFromCurrentPath() {
    ...
    if ($this->currentPathIsValidDuplicatePath()) {
      // By this point, we should be on a Duplicate Node path.
  ...
  /**
   * Determine if the current page path is a valid Duplicate Node path.
   *
   * @return bool
   *   TRUE if valid, FALSE if invalid.
   */
  public function currentPathIsValidDuplicatePath() {
    $path = $this->requestStack->getCurrentRequest()->getRequestUri();
    $path_data = explode('/', $path);

    if (!isset($path_data[1]) || $path_data[1] != 'duplicate') {
      return FALSE;
    }
    ...
    if (!isset($path_data[3]) || $path_data[3] != 'duplicate_node') {
      return FALSE;
    }
    return TRUE;
  }
